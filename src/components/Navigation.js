export default function Navigation(props){
    ...
    return(
        ...
        <>
            ...
            <nav className="sticky top-0 max-h-[90px] px-5 py-4 bg-coal-600 text-xl bg-opacity-80 z-50 block md:hidden">
                ...
                <div id="mobile-nav-panel" className="" >
                    <div className="nav-panel w-screen min-h-full bg-coal-700 absolute flex flex-col justify-between pt-32">
                        <ul className="flex flex-col justify-center font-light">
                            {
                                navLinks.map((el, index) => {
                                    return <li key={`nav_item_mobile-${index}`} className="py-2 my-2" onClick={() => mobileNav()}><a href={el.link}>{el.title}</a></li>;
                                })
                            }
                        </ul>
